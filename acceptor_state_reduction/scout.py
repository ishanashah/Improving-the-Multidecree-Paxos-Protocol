        ...
        while True:
            ...
            if isinstance(msg, P1bMessage):
                if self.ballot_number == msg.ballot_number and msg.src in waitfor:
                    # pvalues.update(msg.accepted.values())
                    for pv in msg.accepted:
                        if (pv.slot_number not in pvalues) or pvalues[
                            pv.slot_number
                        ].ballot_number < pv.ballot_number:
                            pvalues[pv.slot_number] = pv
                    waitfor.remove(msg.src)
                    if len(waitfor) < float(len(self.acceptors)) / 2:
                        self.sendMessage(
                            self.leader,
                            AdoptedMessage(
                                self.id, self.ballot_number, pvalues.values()
                            ),
                        )
                        return
                else:
