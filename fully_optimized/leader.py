from utils import BallotNumber, TIMEOUT_INITIAL, TIMEOUT_MULTIPLIER
from process import Process
from commander import Commander
from scout import Scout
from message import ProposeMessage, AdoptedMessage, PreemptedMessage, SlotOutMessage
import time
import random
        ...
        while True:
            ...
            if isinstance(msg, ProposeMessage):
                ...
                if self.ballot_number == msg.ballot_number:
                    ...
                    #     if (
                    #         pv.slot_number not in pmax
                    #         or pmax[pv.slot_number] < pv.ballot_number
                    #     ):
                ...
                if msg.ballot_number > self.ballot_number:
                    if self.timeout == 0:
                        self.timeout = TIMEOUT_INITIAL
                    else:
                        self.timeout *= TIMEOUT_MULTIPLIER
                    self.active = False
            ...
            elif isinstance(msg, SlotOutMessage):
                for slot in range(self.slot_out, msg.slot_number):
                    if slot in self.proposals:
                        del self.proposals[slot]
                self.slot_out = msg.slot_number - 1
            else:
