...
class Replica(Process):
    ...
    def propose(self):
        while len(self.requests) != 0 and self.slot_in < self.slot_out + WINDOW:
            ...
            if self.slot_in not in self.decisions:
                ...
                self.sendMessage(
                    self.leader, ProposeMessage(self.id, self.slot_in, cmd)
                )
            self.slot_in += 1
    ...
    def perform(self, cmd):
        ...
        self.env.inbox.put(ResponseMessage(self.id, cmd))
        self.slot_out += 1
        self.sendMessage(self.leader, SlotOutMessage(self.id, self.slot_out))
